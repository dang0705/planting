...
- id: code_logic_diagnosis_engine_entry
  type: fact
  statement: 诊断主链执行链路位于 `cloudfunctions/diagnose-http/domain/diagnosis-engine.js` 的 `runDiagnosisRound`，并与 `app/http-router` 的业务处理器组合形成请求闭环。
  source:
...
- id: code_logic_output_contract
  type: fact
  statement: 输出口径优先使用 `visibleOutcomes` / `visibleOutcomeKeys`；`buildPublicRouteFinalResult` 会将既有 `primaryOutcome`/`secondaryOutcomes` 与新可见字段合并并优先保留可显示结果。
  source:
...
- id: data_status_policy
  type: rule
  statement: route/question/symptom/prior 使用 `audited` 与 `partial` 的数据来源策略，必须保持当前既有数据的 `dataStatus` 维持行为。
  source:
    file: cloudfunctions/diagnose-http/domain/diagnosis-engine.js
    symbol: runDiagnosisRound
  verified: 2026-06-05
...
- id: route_authority_decision
  type: decision
  statement: 通过 `isAuthoritativeRouteDecision` 判定“可权威输出”与降级路径（有 `conservativePolicy` 时不视为权威）。
  source:
    file: cloudfunctions/diagnose-http/services/session-result-read-service.js
...
- id: integration_decision
  type: decision
  statement: 采用 `route` 与 `question-selector` 分离职责的架构，统一从 `diagnosis-engine` 与诊断 handlers 串接。
  source:
    file: cloudfunctions/diagnose-http/app/http-router.js
    symbol: route dispatcher
  verified: 2026-06-05
  review_after: 90d
  owner: architecture
  confidence: medium
...
- id: visible_outcomes_public_contract_decision
  type: decision
  adr_id: ADR-002
  statement: `visibleOutcomes` 与 `visibleOutcomeKeys` 是公开主出口，`primaryOutcome`/`secondaryOutcome` 仅用于当前映射，前端显示不得恢复“核心/备选”分区。
  source:
...
- id: route_contract_decision
  type: decision
  statement: route 作为唯一主收敛机制：公开结果由 `routeDecision` 与可见字段共同驱动，非排序头部直接导向 final。
  source:
    file: docs/code-logics/06_问题排序_证据计分_输出守卫.md
    symbol: 本轮 route-only 收敛补充 / 3.1 既有排序口径已废止
  verified: 2026-06-05
  review_after: 90d
  owner: architecture
  confidence: high
...
- id: route_reasoning_entry_decision
  type: decision
  statement: 将主逻辑从“逐题追加问题”切为“候选 outcome + route path + condition 收敛”；公开结果需由 route 收敛链路决定，不以单一分值头部直接返回 final。
  source:
    file: docs/route规划及outcome瘦身计划/00_总览_阅读顺序.md
    symbol: 路由主链从逐题追加切换为 route path 收敛
  verified: 2026-06-05
  review_after: 90d
  owner: architecture
  confidence: high
...
- id: yellowing_path_constraint_decision
  type: decision
  statement: 黄叶入口必须先完成养护/环境实证分组（浇水、光照、施肥、通风/湿度、进展速度）后再可公开 `visibleOutcomes`；单题 `yellowing_care_area_condition` 等聚合题不再作为独立闭合触发口。
  source:
...
- id: max_questions_in_round
  type: observation
  statement: 代码中存在多处 `maxQuestions` 处理链（含高置信流程、候选问题组），默认配置源自 `constants/scoring`，但局部路径可能传入不同上限，需要持续校验。
  source:
