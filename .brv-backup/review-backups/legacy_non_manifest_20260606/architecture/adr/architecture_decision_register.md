...
- id: adr_route_convergence_decision
  type: decision
  adr_id: ADR-001
  statement: 主链从评分系统收敛到 route path 与 `visibleOutcomes` 唯一出口，`ranking` / `scoreGap` 只保留审计意义，不再倒推公开结果。
  source:
...
- id: adr_visible_outcomes_contract_decision
  type: decision
  adr_id: ADR-002
  statement: `visibleOutcomes` 与 `visibleOutcomeKeys` 是公开主出口；`primaryOutcome` / `secondaryOutcome` 仅作兼容映射，不再恢复核心/备选分区。
  source:
    file: .brv/context-tree/docs/code_logics/code_logics_and_architecture.md
