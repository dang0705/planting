...
import { createDiagnoseFlowState } from './state.js'
import {
...
import { useDiagnoseQuestionRisk } from './question-risk.js'
import { useDiagnoseQuestionFlow } from './question-flow.js'
import { useDiagnoseFlowActions } from './popup-actions.js'
...
import { useDiagnoseRetakeSession } from './retake-session.js'
...
function normalizeAdviceGroups(groups) {
  return asArray(groups).map(group => ({
    ...
    showOutcomeLabel: Boolean(group?.showOutcomeLabel),
...
export function setupDiagnoseFlowState(props, { emit, expose }) {
  const ctx = {
    props,
    emit,
    ...deps,
    ...createDiagnoseFlowState(),
  ...
  Object.assign(ctx, useDiagnoseQuestionRisk(ctx))
  Object.assign(ctx, useDiagnoseQuestionFlow(ctx))
  Object.assign(ctx, useDiagnoseFlowActions(ctx))
  Object.assign(ctx, useDiagnoseImages(ctx))
  Object.assign(ctx, useDiagnoseRetakeSession(ctx))
  ...
  const viewModel = {
    popup: ctx.popup,
    result: ctx.result,
    visualScanText: ctx.visualScanText,
    ...
    retakeNow: ctx.retakeNow,
    currentNow: ctx.currentNow,
    retakeAuthorizationReceivedClientAt: ctx.retakeAuthorizationReceivedClientAt,
    ...
    selectedDiagnosisProfile: ctx.selectedDiagnosisProfile,
    ...
    hasRetakeRequest: ctx.hasRetakeRequest,
    hasActiveRetakeAuthorization: ctx.hasActiveRetakeAuthorization,
    retakeCountdownText: ctx.retakeCountdownText,
    retakeExpired: ctx.retakeExpired,
    retakeRequest: ctx.retakeRequest,
    directionChoices: computed(() => asArray(ctx.directionChoices?.value)),
    hasDirectionChoices: ctx.hasDirectionChoices,
    ...
    getQuestionSafetyInstructionsText: question =>
      String(ctx.getQuestionSafetyInstructionsText?.(question) || ''),
    ...
    setDiagnosisProfile: ctx.setDiagnosisProfile,
    ...
    resetImages: () => ctx.uploader.reset(),
    ...
    completeVisualDiagnosis: ctx.completeVisualDiagnosis,
    ...
    beginRetakeAuthorization: ctx.beginRetakeAuthorization,
    skipRetakeRequest: ctx.skipRetakeRequest,
    chooseDirection: ctx.chooseDirection,
    confirmQuestionRisk: ctx.confirmQuestionRisk,
    skipQuestionRisk: ctx.skipQuestionRisk,
    hasQuestionRiskConsent: question => Boolean(ctx.hasQuestionRiskConsent?.(question)),
    isQuestionRiskOptionBlocked: (question, option) =>
      Boolean(ctx.isQuestionRiskOptionBlocked?.(question, option)),
    resetDiagnose: ctx.resetDiagnose,
