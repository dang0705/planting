...
export function buildLightEnvironmentByQuestionIdMap(
  questions = [],
  previousByQuestionId = {},
  savedLightEnvironment = null
) {
  return (Array.isArray(questions) ? questions : [])
    ...
    .reduce((acc, item) => {
      ...
      const source = previousByQuestionId?.[questionId] || savedLightEnvironment
      if (hasMeaningfulLightEnvironment(source)) {
        acc[questionId] = sanitizeLightEnvironment(source)
      }
      return acc
    }, {})
}
...
export function getSavedLightEnvironment(result = {}, plantStore = {}) {
  const resultEnvironment =
    result?.plantContext?.userLightContext ||
    result?.plantContext?.lightEnvironment ||
    result?.userLightContext
  if (hasMeaningfulLightEnvironment(resultEnvironment)) {
    return sanitizeLightEnvironment(resultEnvironment)
  }
  const userPlantId = String(
    result?.plantContext?.userPlantId || result?.userPlantId || result?.plantId || ''
  ).trim()
  const plant = (plantStore?.userPlants || []).find(item => String(item?.id || '') === userPlantId)
  return hasMeaningfulLightEnvironment(plant?.lightEnvironment)
    ? sanitizeLightEnvironment(plant.lightEnvironment)
    : null
}
