<template>
  <view v-if="visible" class="fixed top-0 left-0 right-0 bottom-0 bg-black/70 flex items-center justify-center z-[9999]">
    <view class="bg-white rounded-[20px] w-[85%] max-h-[80vh] overflow-hidden flex flex-col">
      <!-- 标题栏 -->
      <view class="flex items-center justify-between px-5 py-4 border-b border-gray-100">
      ...
      <!-- 内容区域 -->
      <scroll-view scroll-y class="flex-1 px-5 py-4 min-h-[200px] max-h-[400px]">
        <!-- 加载状态 -->
        <view v-if="loading && !streamText" class="flex flex-col items-center py-8">
        ...
        <!-- 流式文本显示 -->
        <view v-else class="text-sm text-gray-700 leading-relaxed">
