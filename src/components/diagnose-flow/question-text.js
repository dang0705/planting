...
export function useDiagnoseQuestionText(ctx) {
  ...
  function getQuestionSafetyInstructionsText(question = {}) {
    const instructions = question?.safetyInstructions
    if (Array.isArray(instructions)) {
      return instructions
        .map(item => sanitizeTemplateText(item))
        .filter(Boolean)
        .join('；')
    }
    return sanitizeTemplateText(instructions || '')
  }
  ...
  return {
    ...
    getQuestionTitle,
    getQuestionHelpText,
    getQuestionSafetyInstructionsText,
    getOptionText,
