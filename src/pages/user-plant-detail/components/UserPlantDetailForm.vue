<template>
  <Layout :title="pageTitle" left-action="back" background-class="bg-[#f8faf9]">
    <view class="min-h-screen bg-[#f8faf9] pb-5">
      <view v-if="isEditMode && loading" class="flex min-h-screen items-center justify-center px-6">
...
async function initializeEditPage() {
  ...
  const image = plant.imageFileId ? await getFileUrl(plant.imageFileId) : ''
...
const environmentPagePaths = Object.freeze({
  light: '/pages/plant-environment/light-environment',
  air: '/pages/airflow/index'
})
...
async function refreshCurrentPlantFromServer() {
  ...
  const image = response.data.imageFileId ? await getFileUrl(response.data.imageFileId) : ''
