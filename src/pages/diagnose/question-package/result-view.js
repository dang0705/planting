...
function formatOutcomeDisplayLabel(outcome = null) {
  if (typeof outcome === 'string') {
    return outcome.replace(/根区压力/g, '根部状态不佳').replace(/根部压力/g, '根部状态不佳').replace(/压力/g, '受影响').trim()
  }
  ...
  return String(
    outcome.displayNameCn ||
      ...
      outcome.outcomeKey ||
      ''
  ).trim().replace(/根区压力/g, '根部状态不佳').replace(/根部压力/g, '根部状态不佳').replace(/压力/g, '受影响').trim()
}
...
function normalizeTextList(values = []) {
  return (Array.isArray(values) ? values : [values]).map(item => String(item || '').trim()).filter(Boolean)
}
...
function buildOutcomeAdviceGroups({ outcomeSources = [], getOutcomeItems, fallbackItems = [], fallbackLabel = '通用建议' } = {}) {
  ...
  if (sourceGroups.length || !fallbackItems.length) {return sourceGroups}
...
export function useQuestionPackageResultView({ result, payload, routeOptions }) {
  const hasCompletedDiagnosis = computed(() => Boolean(result.value) && !result.value.hasActiveQuestions)
  ...
  const nonProblemOutcomeSummaryText = computed(() => String(outcomeSummaryText.value || outcomeTypeText.value || '当前尚未见到明确问题，建议继续观察。').trim())
  ...
  const actionAdviceTexts = computed(() => {
    ...
    const nextSteps = Array.isArray(result.value?.nextSteps) ? result.value.nextSteps.map(item => String(item?.text || '').trim()).filter(Boolean) : []
    ...
    return uniqueStrings([...structuredAdvice, ...(!structuredAdvice.length && treatmentText ? [treatmentText] : [])])
  })
  const avoidAdviceTexts = computed(() => {
    const actionAdvice = result.value?.actionAdvice || {}
    const explanation = result.value?.explanation || result.value?.resultExplanation || {}
    const whatToAvoid = Array.isArray(result.value?.whatToAvoid) ? result.value.whatToAvoid.map(item => String(item || '').trim()).filter(Boolean) : []
    const structuredAdvice = [
    ...
    return uniqueStrings([...structuredAdvice, ...(!structuredAdvice.length && preventionText ? [preventionText] : [])])
  })
  const outcomeAdviceSources = computed(() => buildUniqueOutcomesForAdvice(visibleOutcomeSource.value))
  const actionAdviceGroups = computed(() => buildOutcomeAdviceGroups({
  ...
  const avoidAdviceGroups = computed(() => buildOutcomeAdviceGroups({
    outcomeSources: outcomeAdviceSources.value,
  ...
  const routeDebugEnabled = runtimeEnv.VITE_APP_ENV === 'development' || (Boolean(runtimeEnv.DEV) && runtimeEnv.VITE_APP_ENV !== 'production')
  const routeDebugDecision = computed(() => result.value?.routeDecision || result.value?.__runtimeRouteDecision || null)
  const showRouteDebugPanel = computed(() => routeDebugEnabled && Boolean(routeDebugDecision.value))
  const routeDebugSummaryText = computed(() => String(routeDebugDecision.value?.decisionCause?.decisionCauseText || result.value?.routeDecisionCause?.decisionCauseText || '').trim())
  const routeDebugModeText = computed(() => String(routeDebugDecision.value?.mode || '').trim())
  const routeDebugVisibleOutcomeText = computed(() => normalizeArrayText(routeDebugDecision.value?.visibleOutcomeKeys).join(' / '))
  const routeDebugNextQuestionText = computed(() => normalizeArrayText(routeDebugDecision.value?.nextQuestionKeys).join(' / '))
  const routeDebugGroupText = computed(() => normalizeArrayText(routeDebugDecision.value?.activeRouteGroupKeys).join(' / '))
  const routeDebugFallbackPolicy = computed(() => String(routeDebugDecision.value?.fallbackPolicy || '').trim())
  const sessionLabel = computed(() => String(result.value?.diagnosisSessionId || payload.value?.diagnosisSessionId || routeOptions.value?.sessionId || '未提供 sessionId').trim())
  const roundLabel = computed(() => String(result.value?.roundId || payload.value?.roundId || routeOptions.value?.roundId || '未提供 roundId').trim())
