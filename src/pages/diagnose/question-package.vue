<template>
  <view id="diagnose-question-package-page" class="box-border flex h-screen min-h-screen flex-col bg-[#f8faf9]">
    <template v-if="result?.hasActiveQuestions && questionStack.length">
      <view class="flex min-h-0 flex-1 flex-col pt-6">
        <view class="mb-3 px-4">
          <text class="block text-xl font-extrabold leading-snug text-gray-900">{{ questionDiagnosisContextText }}</text>
          <text class="mt-2 block text-sm font-semibold leading-5 text-[#5a7a68]">{{ questionProgressText }}</text>
        </view>
        ...
        >
          <view class="flex h-full min-h-0 w-full transition-transform duration-[260ms] ease-in-out will-change-transform" :style="questionPageTrackStyle">
            <view
            ...
            >
              <scroll-view :id="`diagnose-question-package-page-question-scroll-${question.questionId || questionIndex}`" scroll-y class="h-full">
                <view :id="`diagnose-question-package-page-question-shell-${question.questionId || questionIndex}`" class="box-border min-h-full px-4 pb-[34px]">
                  <view :id="`diagnose-question-package-page-question-card-${question.questionId || questionIndex}`" class="question-package-card-enter rounded-[20px] border border-emerald-100 bg-white px-4 py-4 shadow-sm">
                    ...
                    >
                      <view
                        ...
                        :class="isSelectedQuestionOption(question, option) ? 'border-[#2d7a4f] bg-emerald-50' : ''"
                    ...
                    <view class="mt-[18px] flex gap-3">
                      <button
                        ...
                        :class="{ 'opacity-[0.45]': isSubmittingQuestionAnswer || activeQuestionIndex <= 0 }"
    ...
    <scroll-view
      ...
      class="h-screen"
    >
      <view id="diagnose-question-package-outcome-shell" class="box-border min-h-screen px-4 py-6 pb-9">
        <view id="diagnose-question-package-outcome-card" class="rounded-[22px] border border-[#e7e0d1] bg-[#fffdf8] p-[18px] shadow-sm">
        ...
        <view v-if="actionAdviceGroups.length" id="diagnose-question-package-outcome-action-advice" class="mt-3.5 rounded-[22px] bg-emerald-50 p-4">
        ...
        <view v-if="avoidAdviceGroups.length" id="diagnose-question-package-outcome-avoid-advice" class="mt-3.5 rounded-[22px] bg-orange-50 p-4">
    ...
    <scroll-view v-else-if="hasCompletedDiagnosis" scroll-y class="h-screen">
      <view id="diagnose-question-package-result-shell" class="box-border min-h-screen px-4 py-6 pb-9">
        <view
        ...
        <view v-if="isProblematicOutcome && allOutcomeDisplays.length" id="diagnose-question-package-result-outcomes" class="mt-3.5 rounded-[22px] border border-[#e7e0d1] bg-[#fffdf8] p-4 shadow-sm">
        ...
        <view v-if="observedItems.length" id="diagnose-question-package-result-observed" class="mt-3.5 rounded-[22px] border border-[#e7e0d1] bg-[#fffdf8] p-4 shadow-sm">
        ...
        <view id="diagnose-question-package-result-action-advice" class="mt-3.5 rounded-[22px] border border-[#e7e0d1] bg-[#fffdf8] p-4 shadow-sm">
        ...
        <view v-if="avoidAdviceGroups.length" id="diagnose-question-package-result-avoid-advice" class="mt-3.5 rounded-[22px] border border-[#e7e0d1] bg-[#fffdf8] p-4 shadow-sm">
        ...
        <view v-if="showRouteDebugPanel" id="diagnose-question-package-debug-panel" class="mt-3.5 rounded-[22px] border border-[#e7e0d1] bg-[#fffdf8] p-4 shadow-sm">
    ...
    <view v-else id="diagnose-question-package-empty-state" class="box-border min-h-screen px-4 py-7">
...
const questionDiagnosisContextText = computed(() => `针对${plantName.value || '植物'}的黄叶问诊`)
