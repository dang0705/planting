...
function resolveYellowingFollowUpOptionText(question = {}, option = {}) {
  if (!isYellowingFollowUpQuestion(question)) {return ''}
...
export function getFollowUpQuestionId(question = {}) {
...
export function getOptionText(question = {}, option = {}) {
  ...
  return resolveYellowingFollowUpOptionText(question, option) || text
}
...
export function useFollowUpQuestionFlow({ result, images, plantName, userStore, diagnoseStore, followUpMutation }) {
  ...
  const isQuestionPackageMode = computed(() => isPackageResult(result.value) && followUpQuestionStack.value.length > 1)
  const followUpPageTrackStyle = computed(() => `transform: translateX(-${activeFollowUpQuestionIndex.value * 100}%);`)
  ...
  const nextButtonText = computed(() => {
    if (isSubmittingFollowUp.value) {return '处理中...'}
    return activeFollowUpQuestionIndex.value >= followUpQuestionStack.value.length - 1 ? '完成问诊' : '下一题'
  })
  ...
  function resetFollowUpQuestionState(followUps = []) {
  ...
  function syncCareBehaviorTimelineAnswer(question, timeline = null) {
    ...
    const currentOptionId = normalizeText(followUpAnswers.value[questionId])
    ...
    const nextAnswerId = hasMeaningfulCareBehaviorTimeline(timeline)
      ...
      : ''
    if (nextAnswerId) {
      if (currentOptionId !== nextAnswerId) {setFollowUpAnswer(questionId, nextAnswerId)}
      return
    }
    if (currentOptionId) {setFollowUpAnswer(questionId, '')}
  }
  ...
  function setFollowUpAnswer(questionId, answerValue) {
    const normalizedQuestionId = normalizeText(questionId)
    if (!normalizedQuestionId) {return}
    followUpAnswers.value = { ...followUpAnswers.value, [normalizedQuestionId]: answerValue }
    const question = followUpQuestionStack.value.find(item => getFollowUpQuestionId(item) === normalizedQuestionId)
  ...
  function isSelectedFollowUpOption(question, option) {
    const questionId = getFollowUpQuestionId(question)
    const optionId = getFollowUpOptionId(option)
    return Boolean(questionId && optionId && normalizeText(followUpAnswers.value[questionId] || question?.defaultOptionId) === optionId)
  }
  ...
  function isQuestionAnswered(question) {
    ...
    if (isCareBehaviorWateringTimelineQuestion(question)) {
      return Boolean(followUpAnswers.value[questionId]) || hasMeaningfulCareBehaviorTimeline(getCareBehaviorTimelineByQuestion(question))
    }
    return Boolean(followUpAnswers.value[questionId])
  }

  function canProceedFollowUpQuestion() {
    if (isSubmittingFollowUp.value || !isQuestionAnswered(currentFollowUpQuestion.value)) {return false}
    if (isQuestionPackageMode.value && activeFollowUpQuestionIndex.value >= followUpQuestionStack.value.length - 1) {
      return followUpQuestionStack.value.every(isQuestionAnswered)
    }
    return true
  }
  ...
  async function submitFollowUps() {
    ...
    try {
      const submitQuestionStack = isQuestionPackageMode.value
        ...
        : currentFollowUpQuestion.value ? [currentFollowUpQuestion.value] : []
  ...
  async function handleNextFollowUpQuestion() {
    ...
    if (activeFollowUpQuestionIndex.value < followUpQuestionStack.value.length - 1) {
