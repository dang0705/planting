...
export function createCatalogLeaf(entry) {
  ...
  if (!legacyScript) {
    const error = new Error(`catalog leaf ${id} is missing config.legacyScript`)
    error.code = 'mp_e2e_contract_invalid'
    throw error
  }
  return defineLeaf(
    { id, dataMode, title: String(config.title || id) },
    context => runCatalogLeaf({ id, legacyScript, sourceDataMode: config.sourceDataMode, context, timeoutMs: config.timeoutMs, environment })
  )
}

async function runCatalogLeaf({ id, legacyScript, sourceDataMode, context, timeoutMs, environment }) {
  ...
  if (!projectRoot || !script.startsWith(`${projectRoot}${path.sep}`)) {
    const error = new Error(`catalog leaf ${id} has an invalid project script path`)
    error.code = 'mp_e2e_contract_invalid'
    throw error
  }
  ...
  if (legacyReport) {execution.legacyReport = summarizeLegacyReport(legacyReport)}
  const executionPath = path.join(context.evidenceDir, 'catalog-leaf-execution.json')
  await writeFile(executionPath, `${JSON.stringify(execution, null, 2)}\n`)
  if (result.error || result.timedOut || result.code !== 0) {
    const legacyFailure = legacyReport?.status === 'failed'
      ? legacyReport.failures?.[0]?.detail || legacyReport.assertions?.find(item => item?.passed === false)?.detail
      : null
    const error = new Error(result.error?.message || legacyFailure || `catalog leaf ${id} failed with exit ${result.code ?? 'unknown'}`)
    ...
    error.details = { executionPath, code: result.code, signal: result.signal, legacyReport: execution.legacyReport || null }
    throw error
  }
  return {
    status: 'passed',
    assertions: [{ name: 'catalog_leaf_exit', passed: true, evidence: { executionPath, code: result.code } }]
  }
}
...
function normalizedEnvironment(value) {
  ...
  for (const [key, entry] of entries) {
    if (!/^[A-Z][A-Z0-9_]*$/u.test(key) || typeof entry !== 'string') {
      const error = new Error('catalog leaf environment must contain uppercase string values')
      error.code = 'mp_e2e_contract_invalid'
      throw error
    }
  }
