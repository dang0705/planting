...
- 任务类型: 高风险诊断流只读审计，涉及 ranking/青花值、route、outcome、condition、问诊路径、runtime。
...
- cloudfunctions/diagnose-http/domain/diagnosis-engine.js:5421: buildFollowUps(...) 仍由既有追问候选池生成。
...
- cloudfunctions/diagnose-http/constants/tables.js:3: 表常量尚无 outcome_routes/outcome_route_conditions/outcome_action_profiles。
...
| display/closure/action safety condition | 既有 output guard + stop reason | condition 未成为统一判定层 |
| route/condition/action profile 表 | 当前仅既有 problems/question_strategy/diagnosis_result_explanations | 数据层未落地 |
...
- 不把黄叶、孔洞、宽泛斑块守卫删掉；只能迁移为 route condition。
...
- 新增 domain/outcome-condition-evaluator.js
...
- 修改 src/utils/diagnose-flow.js 做前端适配归一化

暂不做：

- 删除既有 ranking。
- 删除既有 topProblem/finalResult/rankings 适配字段。
- 生产默认启用 route output。
- 全量 UI 重做。
- 部署或 DB 迁移执行。
...
- 前端若继续消费 topProblem，后端新契约会被 UI 既有逻辑吞掉。
