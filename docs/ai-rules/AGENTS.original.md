...
### CloudBase Function Deployment

- Do NOT use `npx @cloudbase/cloudbase-mcp@latest updateFunctionCode ...` as a CLI deployment command.
- That package is not a reliable CloudBase CLI entrypoint; it previously returned success while `diagnose-http` code was not updated.
- Use the project deployment wrapper:
  - `node scripts/deploy-function.js diagnose-http`
- The wrapper must use CloudBase CLI code update:
  - `npx --package @cloudbase/cli@3.2.2 tcb fn code update <functionName> --dir cloudfunctions/<functionName> -e <envId> --json`
- Prefer `tcb fn code update` over `tcb fn deploy` for existing functions, because it updates code only and avoids overwriting runtime config, timeout, or envVariables.
- After publishing, always confirm real deployment with `tcb fn detail <functionName> -e <envId>` and verify `Modification time` / `Code size` changed.
- For `diagnose-http`, a publish is not accepted until a real HTTP smoke and, when relevant, `diagnosis_sessions` / `diagnosis_follow_ups` DB evidence show the new behavior.
- Detailed project note: `docs/cloud-function-deployment-guide.md`.
