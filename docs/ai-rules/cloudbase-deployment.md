...
部署、SQL、查诊断 session 等 CloudBase 相关任务，优先使用当前会话可用的 CloudBase MCP 工具。
但 `diagnose-http` 是版本路由敏感函数：发布必须使用项目统一门禁脚本；不能用通用 MCP
函数更新或其他直传方式替代，否则无法保证 `$DEFAULT`、代码摘要和真实网关 qualifier 一致。
项目 wrapper 仅作为统一脚本的 fallback / convenience path，不能单独作为发布通过证据。
...
wrapper 内部转入 `scripts/deploy-cloudbase-functions.mjs`，不得绕过版本发布门禁。
其中普通函数使用 CloudBase CLI code update；`diagnose-http` 还必须完成不可变版本与
`$DEFAULT` 别名切换：
...
5. `diagnose-http` 的 `$DEFAULT` 必须指向数字版本，不能指向 `$LATEST`；发布脚本会在灰度路由、摘要不一致、别名未切换或网关仍落到旧 qualifier 时失败并回退。
6. 若涉及 ranking → route、outcome 瘦身、condition、问诊路径、replay，必须结合 `docs/ai-rules/diagnosis-replay.md` 的验证要求。
7. 若涉及诊断 `fast path`、`warm path`、`early return`、缓存命中或性能优化路径，发布验收必须包含“应继续追问而非 final”的真实 `/diagnosis/answer` 负向 smoke、完整路径正向 smoke，以及对应 DB 状态证据；部署命令成功或函数详情正常不能替代该验收。
