...
| 首页浇水提醒入口         | `src/pages/index/components/PlantCard.vue` | `plant-card-reminder-{plant.id}-water`                                                               | 点击打开浇水提醒弹框；断言水滴提醒状态                                    |
| 浇水提醒弹框             | `src/pages/index/components/WateringReminderSheet.vue` | `watering-reminder-sheet` / `watering-reminder-confirm-button`                                       | 断言弹框打开；点击添加到手机日历                                          |
| 已保存浇水提醒回显       | `src/pages/index/components/WateringReminderSheet.vue` | `watering-reminder-saved-state` / `watering-reminder-saved-next-time` / `watering-reminder-saved-reason` | 断言已保存提醒、下次浇水建议与原因回显                                    |
