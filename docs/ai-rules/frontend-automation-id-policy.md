...
| 植物列表加载骨架    | `src/pages/index/index.vue`                                          | `index-plants-loading-skeleton`                                                                                                                                                                                                                                                                                                                                                                           | 断言用户植物数据加载期间不提前展示空态                        |
...
| 空气环境未知     | `src/subpackages/diagnosis/question-package.vue` / `AirEnvironmentQuestionInput.vue` | `diagnose-air-environment-{questionId}-unknown`                                                              | 固定在题包底部；仅将当前空气题标为不确定，不能附带 sidecar |
...
`PotProfileFormCore` 的拖拽节点与选项使用 `idPrefix` 前缀；首页浇水提醒步骤使用 `watering-reminder-input-pot-profile`，其他入口按页面前缀生成。

| 功能模块        | 组件                                                | 稳定 id 后缀                   | 操作 / 断言                  |
| --------------- | --------------------------------------------------- | ------------------------------ | ---------------------------- |
| 盆口/盆高拖拽点 | `src/components/PotCanvas.vue`                      | `{idPrefix}-top-handle`        | 拖动同时调整盆口直径与盆高   |
| 盆底拖拽点      | `src/components/PotCanvas.vue`                      | `{idPrefix}-bottom-handle`     | 拖动调整盆底直径             |
...
| `care.watering.reminder_pot_profile.persistence`    | `test/e2e/automator/care/watering/reminder-pot-profile/persistence.mjs`      | `3.5`、`3.5.1`      |
