...
启动本地 Swagger 风格接口文档页：

```bash
npm run docs:api
```

在浏览器打开 `http://127.0.0.1:4177`。页面按业务分类展示当前 HTTP 接口、入参与返回的业务含义，并可经本机代理调用本地 gateway。它只读取 `.env.local` 中白名单内的开发连接信息（端口、环境和开发身份）；不会读取或展示任何密钥。远端调试需临时填入用户自己的会话令牌，页面不会保存该令牌。

文件上传、支付回调和运营审查接口会显示参数说明，但分别需要二进制上传、支付平台签名或运营权限，不能用普通 JSON 请求替代真实链路。
