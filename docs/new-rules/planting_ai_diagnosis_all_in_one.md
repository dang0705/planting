...
```text
最明显线索 gate / 养护方向 gate / 虫害线索 gate / 病斑霉层线索 gate / 发黄分布模式
```
...
若未来恢复新叶/老叶问题，必须先经过“新叶更明显 / 老叶或下部叶更明显”的二选一 gate 且用户明确选择；或由用户补拍照片，经视觉模型形成可靠的新叶/老叶结论后，才可继续进入各自的新叶或老叶后续问题。没有该 gate 选择或视觉结论时，不能展示“老叶是否均匀变黄”这类带方向诱导的问题。
...
- 养护变化 / 病虫害线索 / 病斑霉层线索 / 只有黄叶或不确定
- 脉间黄化 / 整片黄化
- 湿土萎蔫 / 干土萎蔫
- 是否伴随根系异常
- 是否伴随弱光 / 温度 / 施肥历史

新叶黄 / 老叶黄当前不作为黄叶问诊流程的优先区分项；只允许作为 outcome 建议中的观察提示。未来恢复时，必须先经二选一 gate 或补拍视觉确认，不能直接作为首题或默认分流。
...
- 黄叶第一层 gate 后再进入对应方向：养护方向先问带短标题和 description 的养护项 gate，再按答案问浇水频率、直射时长、施肥次数或通风湿度；虫害方向问可见虫痕；病害方向问黄晕、水渍感、霉层/粉层；只有黄叶或不确定时只能回退到允许的分布、养护、病虫害、病斑霉层或进展速度题，不得进入新/老叶或叶龄分流。
...
| `nitrogen_deficiency` | `fertilization_gap`、施肥/生长背景；整体均匀黄化只能作为弱线索，不能单独放行缺氮 |
| `iron_deficiency` | 叶脉间失绿、长期缺肥/介质问题；新叶黄化只能作为视觉弱线索，不能在当前黄叶问诊中追问 |
| `nutrient_deficiency` | `fertilization_gap`、施肥/生长背景；整体均匀黄化只能作为弱线索，不能单独放行营养不足 |
...
- `uncertain` 的原因应指向“缺少正向上下文证据”，并追加复杂性提示：黄叶原因通常不止一种，除当前观察方向外应复核叶背、叶柄和叶面是否有小虫、细网、黑点、发黏、斑点或霉粉；若主要是底部老叶缓慢变黄且新叶正常，需考虑老叶自然代谢背景。该提示只作为复核建议，不生成额外 outcome。
...
- 先区分养护变化、病虫害线索、病斑霉层线索，以及只有黄叶或不确定
- 先区分脉间黄化 / 整片黄化
- 先区分湿土萎蔫 / 干土萎蔫
- 再细分缺铁、缺氮、缺镁、弱光、根系问题等

新叶黄 / 老叶黄当前不进入黄叶问诊流程；若未来恢复，必须先经“新叶更明显 / 老叶或下部叶更明显”二选一 gate 或补拍视觉确认，再进入对应后续问题。
