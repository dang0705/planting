...
| schema | diagnosis_outcomes | route_groups | routes | route_conditions | route_questions | answer_effects | action_profiles |
|---|---:|---:|---:|---:|---:|---:|---:|
...
| `structural_damage_old_injury` | 结构损伤/既有伤 | 1 | `holes_old_injury_route` |
...
| route | 到达 outcome | route 条件 / condition | route 追问 / answer effect |
|---|---|---|---|
| `holes_chewing_pest_route` 孔洞 + 虫迹路径 | `chewing_pest_damage` 疑似虫害痕迹 | `chewing_pest_condition` 要求 `holes_in_leaf + visible_pest_trace` | `q_holes_in_leaf_confirm:yes` 或 `q_observed_probe__holes_in_leaf__structural_cause:pest_like` 支持该路径 |
| `holes_old_injury_route` 孔洞 + 无虫迹 + 不扩散路径 | `structural_damage_old_injury` 结构损伤/既有伤 | `old_injury_condition` 要求 `holes_in_leaf + no_pest_trace + not_spreading`，阻断 `visible_pest_trace` | `q_observed_probe__holes_in_leaf__structural_cause:old_injury` 支持该路径 |
...
| route | 到达 outcome | route 条件 / condition | route 追问 / answer effect |
|---|---|---|---|
| `leaf_spot_humid_route` 斑点扩散 + 通风差路径 | `leaf_spot_problem` 叶斑类问题 | `leaf_spot_condition` 要求 `spreading_spots + poor_ventilation` | 当前无 active route question / answer effect；依赖正式症状证据闭合 |
...
| route | 到达 outcome | route 条件 / condition | route 追问 / answer effect |
|---|---|---|---|
| `sunburn_recent_exposure_route` 焦斑 + 最近暴晒路径 | `sunburn` 晒伤/强光刺激 | `sunburn_condition` 要求 `scorching_spots + recent_strong_sun` | 当前无 active route question / answer effect |
| `leggy_low_light_route` 徒长 + 弱光路径 | `low_light_growth_weakness` 光照不足/生长偏弱 | `low_light_condition` 要求 `leggy_growth + weak_light` | 当前无 active route question / answer effect |
| `dry_air_leaf_edge_route` 焦边 + 干空气路径 | `dry_air_stress` 叶尖焦枯/环境压力 | `dry_air_condition` 要求 `burnt_leaf_edge + dry_air_condition` | 当前无 active route question / answer effect |
...
| route | 到达 outcome | route 条件 / condition | route 追问 / answer effect |
|---|---|---|---|
| `wilting_wet_soil_route` 萎蔫 + 土湿路径 | `overwatering_root_pressure` 积水/根系压力 | `wilting_wet_condition` 要求 `wilting` 且 `q_root_rot_wet_soil_wilt:yes`，阻断 `:no` | 追问 `q_root_rot_wet_soil_wilt` |
| `wilting_dry_soil_route` 萎蔫 + 土干路径 | `underwatering` 缺水压力 | `wilting_dry_condition` 要求 `wilting` 且 `q_root_rot_wet_soil_wilt:no`，阻断 `:yes` | 追问 `q_root_rot_wet_soil_wilt` |
...
默认前置题：`watering_frequency_context`、`light_change_context`、`fertilization_growth_context`、`airflow_humidity_context`；不再以 `q_observed_probe__leaf_yellowing__yellowing_care_area_condition` 作为首题或单点聚合 condition。纯黄叶入口不得把 `leaf_spot_problem`、叶斑、病斑、斑纹、mosaic 或虫害题混入后续问诊。
...
| `yellowing_old_leaf_route` 黄叶 + 底部老叶自然代谢路径 | `normal_leaf_aging` 自然代谢 | 历史适配路径保留；纯黄叶入口当前不主动追问 `yellowing_leaf_age_pattern`，只能在已有可靠证据时参与收敛 | 不作为纯黄叶首轮追问 |
...
1. 自然业务理解：室内植物诊断不能把单一泛化症状直接闭合为具体 outcome，必须由 route/condition 提供更高特异事实。
...
但有 3 类 route 需要更严格的 condition 或文档标注：

| route | 当前判断 | 建议 |
|---|---|---|
| `yellowing_airflow_root_stress_route` | 偏弱。黄叶 + 萎蔫/掉叶可以指向根部环境压力，但同样可能是过干、过湿、低湿、温度/环境突变。 | 建议后续 SQL condition 增加土壤持续偏湿、根部异常、排水差、盆土问题等根区证据；否则应降级到 `uncertain_observation` 或 `humidity_airflow_stress`。 |
...
| `holes_in_leaf_split_group` | 合理 | Illinois Extension 指出孔洞、叶片缺损、叶片被吃掉通常可提示咀嚼类昆虫；无虫迹且不扩散时转既有伤/结构损伤合理。 |
...
2. `dry_air_leaf_edge_route`：增加低湿/干空气/风口类上下文或排除肥害/水分异常的 condition。
...
- 关键 requestId：
  ...
  - production condition 明细：`c04ad271-b8c0-46fc-b4fe-68b2ca8447fc`
...
- 本轮变更确认 `development route` 的输出不以 ranking/青花值排名为依赖，全部以 `候选 outcome` + `路径 condition` 规则收敛。
