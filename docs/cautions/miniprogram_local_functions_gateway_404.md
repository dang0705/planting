...
6. 小程序本地启动脚本对 `plant-user-http/user-plants` 和 `weather-http/weather/current` 做轻量业务探针，避免 3010 上已有既有 gateway 时只看 health 200 而漏掉首页业务 500。
...
如果验收业务接口而不是 health route，必须先在未提交的 `.env.local` 或 shell 中配置已轮换的最小权限 CloudBase 凭据。`CLOUDBASE_*`、`TENCENT_*`、`TENCENTCLOUD_*` 三套命名都可用；不要使用已泄漏的既有密钥。
