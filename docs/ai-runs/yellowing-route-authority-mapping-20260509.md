...
> 2026-05-16 口径修正：下表保留 2026-05-09 的历史映射快照。运行时不再把 `yellowing_primary_clue_condition + yellowing_care_area_condition` 作为一屏聚合 condition 闭合条件；黄叶类 condition 现在按分组逐页提问，一页只展示一组 `condition.options`，累计所有必要分组答案后再由 `route/condition/outcome` 收敛。

| 权威来源 | 观察/判断依据 | SQL route / condition | question-option 闭环 | outcome |
|---|---|---|---|---|
| RHS `Leaf damage on houseplants` | 黄叶先看文化条件，不先默认病虫害；过湿根压优先看长期过湿背景 | `yellowing_wet_soil_route` / `wet_soil_confirmation_condition` | `yellowing_primary_clue_condition=care_context` + `yellowing_care_area_condition=watering_area` + `watering_frequency_context=often_wet` | `overwatering_root_pressure` |
| RHS `How to help a poorly houseplant` | 缺水与过浇都可黄化/萎蔫，必须结合盆土与浇水背景区分 | `yellowing_dry_soil_route` / `dry_soil_confirmation_condition` | `yellowing_primary_clue_condition=care_context` + `yellowing_care_area_condition=watering_area` + `watering_frequency_context=often_dry` | `underwatering` |
| RHS `Leaf damage on houseplants` + RHS `How to help a poorly houseplant` | 下位老叶先黄、稳定脱落，可作为自然代谢 | `yellowing_old_leaf_route` / `old_leaf_aging_condition` | `yellowing_leaf_age_pattern=old_lower_leaves_first` | `normal_leaf_aging` |
| Clemson `Indoor Plants - Watering` + UC IPM `Houseplant Problems` | 阴处需水更少，弱光会黄化/弱生长；光照偏弱的黄叶需从养护分流闭环 | `yellowing_low_light_route` / `yellowing_low_light_condition` | `yellowing_primary_clue_condition=care_context` + `yellowing_care_area_condition=light_area` + `light_change_context=weaker_light` | `low_light_growth_weakness` |
...
1. `q_observed_probe__leaf_yellowing__yellowing_primary_clue_condition`
2. `q_observed_probe__leaf_yellowing__yellowing_care_area_condition`
3. 进入更细的上下文题：
...
- 黄叶入口成立
- `yellowing_primary_clue_condition = care_context`
- `yellowing_care_area_condition = watering_area`
...
- 黄叶入口成立
- `yellowing_primary_clue_condition = care_context`
- `yellowing_care_area_condition = light_area`
...
2. `yellowing_wet_soil_route` / `yellowing_dry_soil_route` / `yellowing_old_leaf_route` 的 condition 改为可消费人工链 question-option。
...
4. 当 `care_context -> watering_area -> often_wet` 成立时，最终公开结果必须能闭合到 `overwatering_root_pressure`，而不是落回既有的 `uncertain_output_ready`。
