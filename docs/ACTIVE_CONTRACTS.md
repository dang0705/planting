...
D0 now 采样改为 per-city sunrise/sunset trigger：日出触发器为 `weather-d0-now-sunrise__{safeLocationKey}`，日落触发器为 `weather-d0-now-sunset__{safeLocationKey}`（示例：`weather-d0-now-sunrise__city_shanghai`、`weather-d0-now-sunset__city_shanghai`），旧单 `weather-d0-now-sunrise` 和固定 `weather-d0-now-finalize-2130` 不再作为精确触发依据。运行态存到 `weather-cache/v1/season-trigger-state/{safeLocationKey}.json`，审计落在 `weather-cache/v1/season-trigger-audit/{safeLocationKey}/{year}.jsonl`。动态触发器由 `suncalc` 结合节气历同步后按城市日出/日落计算。其他 D0 timer 保持 `weather-d0-now-morning-0920`、`weather-d0-now-forenoon-1220`、`weather-d0-now-noon-1420`、`weather-d0-now-afternoon-1820`。CloudBase trigger 写入必须按完整 timer 集合提交：固定 5 个基线 + state 中仍有效的 per-city sunrise/sunset；不得只提交固定基线或单个动态 trigger。同一城市 sunrise 或 sunset 漂移 `>=15` 分钟才更新。真实 slot 规则由 `now-sample-slots` 计算：morning=`sunrise`、forenoon=`12:20`、noon=`14:20`、afternoon=`sunset+20m`、finalize=`sunset`；固定 cron 只负责中间采样唤醒，不能替代 sunrise/sunset 语义。`weather-ingestion-recent-10d` 只维护 D-1 到 D-10 recent 窗口，不负责创建当天 `days/{date}.json`。
