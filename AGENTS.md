...
9. 用户界面安全与展示边界是硬约束：任何情况下，页面、弹窗、Toast、错误提示、分享/导出/复制内容都不得把内部字段名、数据库主键、会话/请求/追踪 ID、内部 key/code/status、密钥、令牌、Cookie、OpenID/UnionID 或其他平台身份标识、模型提示词/推理内容、原始接口响应，以及不属于当前用户且未经授权的业务用户信息直接或间接展示给用户。不得使用 `id`、`key`、`code`、`status` 等内部字段作为任何用户文案、标签、标题、选项或错误提示的兜底值；缺少合法展示文案时必须隐藏该项或使用经过确认的通用提示。后端公开响应、前端归一化、页面计算和组件模板必须分层执行内部字段与展示字段的转换，并对空值、未知值、旧数据和异常响应做反向验证；用户可见文案必须从用户角度出发、符合常识、便于操作，严禁暴露内部讨论用语、计算公式、调试信息或拗口难懂的内容。只有经过明确产品定义、脱敏和安全审查的业务展示编号，才可作为专门的用户文案展示，不得复用内部 ID。
