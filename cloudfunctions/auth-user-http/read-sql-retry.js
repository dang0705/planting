...
function isTransientCloudbaseSqlConnectionError(error) {
  ...
  // RunMysqlCommand may wrap a transient connection failure as InvalidParameter.
  // The message must still contain an explicit connection-failure signal; do not
  // retry every InvalidParameter or every generic query failure.
  return /(?:database connection failed|connection (?:reset|lost|refused|unavailable|error)|socket hang up|timed out|timeout)/iu.test(
    message
  )
}
