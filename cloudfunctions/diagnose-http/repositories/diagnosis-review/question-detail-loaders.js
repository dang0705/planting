...
async function listDiagnosisReviewQuestions(diagnosisSessionId = '') {
  ...
  const result = await models.$runSQL(
    `
      SELECT
        questions.id,
        questions.diagnosis_id,
        questions.question_order,
        questions.symptom_key,
        questions.question_text,
        questions.rationale,
        questions.asked,
        questions.answer_value,
        questions.answer_confidence,
        questions.status,
      ...
      FROM ${table('diagnosis_follow_ups')} AS questions
      LEFT JOIN ${table('question_library_v5_real')} AS questions
        ON questions.question_key = questions.symptom_key
      LEFT JOIN ${table('question_option_mapping_v5_real')} AS options
        ON options.question_key = questions.symptom_key
       AND options.option_key = questions.answer_value
      WHERE questions.diagnosis_id = {{diagnosisSessionId}}
      ORDER BY questions.question_order ASC, questions.id ASC
      LIMIT 20
    `,
  ...
  const resolvedMappingIndex = indexSyntheticQuestionOptionMappings([
    ...auditedOptionMappings,
    ...buildSyntheticQuestionOptionMappings(questionKeys, symptomDictionary)
  ])
