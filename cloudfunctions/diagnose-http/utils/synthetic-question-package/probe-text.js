...
function buildOrthogonalProbeText(item = {}, packageTopic = '', _context = {}) {
  ...
  switch (packageTopic) {
    case QUESTION_PACKAGE_TOPICS.YELLOWING_PRIMARY_CLUE_TOPIC:
      return {
    ...
    case QUESTION_PACKAGE_TOPICS.YELLOWING_CARE_AREA_TOPIC:
      return {
    ...
    case QUESTION_PACKAGE_TOPICS.STRUCTURAL_CAUSE:
      return {
        questionText: `这题是为了区分真实缺损更像虫害、病斑脱落，还是既有伤/摩擦，不是重复确认有没有洞。请看这些“${symptomLabel}”周围更像哪种情况？`,
        helpText: '这题不是确认有没有缺损，而是区分缺损更像虫害痕迹、病斑脱落，还是机械/既有伤。',
        packageSection: QUESTION_PACKAGE_SECTIONS.DIFFERENTIAL_PROBE
      }
    case QUESTION_PACKAGE_TOPICS.PEST_TRACE_TYPE:
      return {
        questionText: '这题是为了确认有没有更直接的虫子活动线索，避免只凭黄点、缺口或斑驳就判断成虫害。这些痕迹旁边更接近哪种情况？',
        helpText: '这题用于区分红蜘蛛/螨类、蓟马、蜜露类刺吸害虫和非虫害痕迹，不把黄点或银斑直接当虫害。',
        packageSection: QUESTION_PACKAGE_SECTIONS.DIFFERENTIAL_PROBE
      }
    case QUESTION_PACKAGE_TOPICS.EDEMA_BUMP_STAGE:
      return {
    ...
    case QUESTION_PACKAGE_TOPICS.LEAF_TUNNEL_PATTERN:
      return {
        questionText: '线状痕迹需要先区分是不是叶片内部的“潜叶道”，否则容易和划痕、既有伤或反光混淆。这些线状痕迹更像叶片里面弯曲延伸的浅白隧道，还是表面的划痕、既有伤或反光？',
        helpText: '潜叶道通常像在叶肉内部延伸的弯曲浅色线，不等同于普通孔洞或表面划痕。',
        packageSection: QUESTION_PACKAGE_SECTIONS.DIFFERENTIAL_PROBE
      }
    case QUESTION_PACKAGE_TOPICS.POWDER_PATTERN:
      return {
        questionText: '白色粉层要先看是否在扩散，才能区分活跃粉层和较稳定的表面残留。这些白色粉层最近更像在叶面逐渐扩散，还是只停留在少数固定位置？',
        helpText: '这题不要求擦拭白粉，只题目分布和扩散方式，用来区分活跃粉层和不活跃表面异常。',
        packageSection: QUESTION_PACKAGE_SECTIONS.DIFFERENTIAL_PROBE
      }
    case QUESTION_PACKAGE_TOPICS.SURFACE_STICKINESS:
      return {
        questionText:
          locationKey === 'leaf'
    ...
    case QUESTION_PACKAGE_TOPICS.TISSUE_INTEGRITY:
      return {
        questionText: locationKey === 'leaf'
          ? `这些“${symptomLabel}”的位置是不是真的破了洞或缺了一块？`
          : `这些“${symptomLabel}”的位置有没有真的破损或缺了一块？`,
        helpText: '只确认是否真的破损或缺失；表面变色、斑点、焦边或既有伤痕迹不算真实缺口。',
        packageSection: QUESTION_PACKAGE_SECTIONS.DIFFERENTIAL_PROBE
      }
    case QUESTION_PACKAGE_TOPICS.SURFACE_RESIDUE:
      return {
        questionText: locationKey === 'leaf'
    ...
    case QUESTION_PACKAGE_TOPICS.LESION_HALO:
      return {
        questionText: locationKey === 'leaf'
          ? '这些斑点周围是否还能看到一圈偏黄、发浅或像晕开的边缘？'
          : `这些“${symptomLabel}”周围是否还有一圈更浅色、像晕开的边缘？`,
        helpText: '这一题不是重复确认黑斑本身，而是题目是否伴随黄晕，这会影响叶斑问题的分流方向。',
        packageSection: QUESTION_PACKAGE_SECTIONS.DIFFERENTIAL_PROBE
      }
    case QUESTION_PACKAGE_TOPICS.LESION_WATER_SOAKING:
      return {
        questionText: locationKey === 'leaf'
          ? '斑点边缘有没有像被水浸过一样，发暗、半透明或偏软？'
          : `这些“${symptomLabel}”边缘有没有像被水浸过一样，发暗、半透明或偏软？`,
        helpText: '这一题用来题目病斑边缘是否有水浸/湿软特征，帮助区分不同叶斑路径。',
        packageSection: QUESTION_PACKAGE_SECTIONS.DIFFERENTIAL_PROBE
      }
    case QUESTION_PACKAGE_TOPICS.DISTRIBUTION_SCOPE:
      return {
        questionText: locationKey === 'leaf'
    ...
    case QUESTION_PACKAGE_TOPICS.UNDERSIDE_PRESENCE:
      return {
        ...
        helpText: isStructuralChewingSymptom(item)
          ? '结构缺损本身不是虫害结论；这一题只题目是否有更直接的虫害线索。'
          : '很多叶部问题在叶背、叶脉夹角或隐蔽部位更容易暴露真实线索。',
        packageSection: QUESTION_PACKAGE_SECTIONS.DIFFERENTIAL_PROBE
      }
    case QUESTION_PACKAGE_TOPICS.PROGRESSION:
      return {
        questionText: `最近 7 天内，这类“${symptomLabel}”有没有明显变多、变大或加重？`,
        helpText: '进展速度会影响是活跃过程、环境事件，还是较稳定的既有损伤/既有斑。',
        packageSection: QUESTION_PACKAGE_SECTIONS.CONTEXT_PROBE
      }
    case QUESTION_PACKAGE_TOPICS.HOST_CONFIRMATION:
    ...
    case QUESTION_PACKAGE_TOPICS.LIGHT_EXPOSURE:
      return {
        questionText:
          locationKey === 'leaf'
    ...
    case QUESTION_PACKAGE_TOPICS.WATERING_CONTEXT:
      return {
        questionText:
          locationKey === 'leaf'
    ...
    case QUESTION_PACKAGE_TOPICS.FERTILIZATION_CONTEXT:
      return {
