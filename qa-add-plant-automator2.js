...
async function run() {
  ...
  const firstScreenReqs = await miniProgram.evaluate(() =>
    JSON.parse(JSON.stringify(globalThis.__qaRequests || []))
  )
  log('FIRST_SCREEN_REQUESTS: ' + JSON.stringify(firstScreenReqs))

  const firstUrls = firstScreenReqs.map(r => r.url)
  const p1_hasCatalog = firstUrls.some(
    u => u.includes('plant-catalog') || u.includes('catalog/plants')
  )
  ...
  const p1_hasUserPlants = firstUrls.some(
    u => u.includes('user-plants') || u.includes('plant-user')
  )

  log(
    'POINT1_CHECK: catalog=' +
      p1_hasCatalog +
      ' hotCities=' +
      p1_hasHotCities +
      ' resolve=' +
  ...
  log(
    'POINT1_RESULT: ' +
      (p1_hasCatalog && !p1_hasHotCities && !p1_hasResolve && !p1_hasAuthUser && !p1_hasUserPlants
        ? 'PASS'
        : 'FAIL')
  )
  ...
  const nextHasBorder0 =
    nextBtnInfo &&
    nextBtnInfo.className &&
    String(nextBtnInfo.className).includes('after_cborder-0')
  const aiHasBorder0 =
    aiBtnInfo && aiBtnInfo.className && String(aiBtnInfo.className).includes('after_cborder-0')
  log(
    'POINT4_CHECK: nextBtn_after_cborder-0=' +
      nextHasBorder0 +
      ' aiBtn_after_cborder-0=' +
      aiHasBorder0
  )
  log(
    'POINT4_RESULT: ' +
      (nextHasBorder0 && aiHasBorder0 ? 'PASS' : nextHasBorder0 ? 'PARTIAL' : 'FAIL')
  )
  ...
  if (plantSelected) {
    ...
    try {
      ...
      if (nb) {
        nextBtnAfter = {
          disabled: await nb.attribute('disabled'),
    ...
    const isDisabled =
      nextBtnAfter && (nextBtnAfter.disabled === 'true' || nextBtnAfter.disabled === true)
    if (!isDisabled) {
    ...
    const allReqs = await miniProgram.evaluate(() =>
      JSON.parse(JSON.stringify(globalThis.__qaRequests || []))
    )
  ...
  const p2e_hasUserPlants = editUrls.some(
    u => u.includes('user-plants') || u.includes('plant-user')
  )
