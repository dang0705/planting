...
export const engineColumns = [
  ...
  'package_topic_default',
  'package_section_default',
...
export function createClosureBatchToolkit({
  ...
  function normalizeQuestion(closure, question, kind) {
    ...
    return {
      ...
      packageTopic: question.packageTopic || 'visual_presence',
      packageSection: question.packageSection || 'symptom_confirmation',
      allowUnknown: 1,
  ...
  function buildQuestionLibrarySql(questionRows = []) {
    const values = questionRows.map(row => [
      ...
      sqlString(row.packageTopic),
      sqlString(row.packageSection),
  ...
  function buildEngineRows(questionRows = []) {
    return questionRows.map(row => ({
      ...
      packageTopicDefault: row.packageTopic || 'visual_presence',
      packageSectionDefault: row.packageSection || 'symptom_confirmation',
  ...
  function buildEngineSql(engineRows = []) {
    const values = engineRows.map(row => [
      ...
      sqlString(row.packageTopicDefault),
      sqlString(row.packageSectionDefault),
