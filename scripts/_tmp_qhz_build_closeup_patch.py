...
SEL=json.load(open(ROOT/'scripts/_tmp_qhz_closeup_replacements.json',encoding='utf-8'))
OUT=ROOT/f"_tmp_qhz_closeup_patch_{os.environ.get('PART_INDEX','0')}"
API='https://commons.wikimedia.org/w/api.php'
S=requests.Session();S.headers.update({'User-Agent':'QinghuazhiCloseupPatch/1.0 (GitHub dang0705/planting)'})
...
def download(url,tries=7):
 ...
 for i in range(tries):
  try:
   ...
   if i==tries-1:raise
   time.sleep(wait);wait=min(wait*1.8,18)
def catalog():
 d={}
...
def main():
 ...
 OUT.mkdir(exist_ok=True);records=[];fail=[]
 for n,pid in enumerate(ids,1):
  plant=cats[pid];title=SEL[pid];print(f'[{n}/{len(ids)}] {pid} {plant["name"]}',flush=True)
  try:
   data=get({'action':'query','format':'json','formatversion':'2','titles':title,'prop':'imageinfo','iiprop':'url|extmetadata|size|mime','iiurlwidth':1600});pages=data.get('query',{}).get('pages',[])
   ...
   safe=re.sub(r'[^a-z0-9]+','-',plant['scientific'].lower().replace('×','x')).strip('-');sub=CATDIR[plant['cat_cn']];(OUT/sub).mkdir(parents=True,exist_ok=True);rel=f'{sub}/{pid}_{safe}.jpg';im.save(OUT/rel,'JPEG',quality=91,optimize=True,progressive=True)
   ...
   records.append({'plant_id':pid,'category':plant['cat_cn'],'plant_cn':plant['name'],'scientific_name':plant['scientific'],'commons_title':actual,'source_page':'https://commons.wikimedia.org/wiki/'+actual.replace(' ','_'),'original_url':orig,'author':author,'license':lic,'license_url':license_url(lic),'needs_attribution':'no' if ('cc0' in lic.lower() or 'public domain' in lic.lower()) else 'yes','source':source,'description':desc,'original_width':ii.get('width'),'original_height':ii.get('height'),'file':rel})
  except Exception as e:
   fail.append({'plant_id':pid,'title':title,'error':str(e)});print(' FAIL',e,flush=True)
  time.sleep(.55)
 ...
 json.dump({'requested':len(ids),'downloaded':len(records),'failed':len(fail),'records':records,'failures':fail},open(OUT/'manifest.json','w',encoding='utf-8'),ensure_ascii=False,indent=2)
