...
UA='QinghuazhiMVPFinalAssetBuilder/2.1 (GitHub dang0705/planting)'
...
def get(params,tries=3,timeout=25):
    wait=0.5
    for i in range(tries):
        try:
            r=S.get(API,params=params,timeout=timeout)
            if r.status_code==429:
                time.sleep(float(r.headers.get('Retry-After') or wait)); wait=min(wait*2,4); continue
            r.raise_for_status(); return r
        except Exception:
            if i==tries-1: raise
            time.sleep(wait); wait=min(wait*2,4)
...
def download(url,tries=3):
    ...
    for i in range(tries):
        try:
            ...
            if r.status_code==429:
                time.sleep(float(r.headers.get('Retry-After') or wait)); wait=min(wait*2,5); continue
            r.raise_for_status(); return r.content
        except Exception:
            if i==tries-1: raise
            time.sleep(wait); wait=min(wait*2,5)
...
def main():
    ...
    for n,pid in enumerate(ids,1):
        ...
        try:
            ...
            raw=download(url); im=Image.open(io.BytesIO(raw)); im=ImageOps.exif_transpose(im).convert('RGB')
            ow,oh=im.size; im.thumbnail((1800,1800),Image.Resampling.LANCZOS)
            sub=CATDIR[plant['cat_cn']]; (OUT/sub).mkdir(parents=True,exist_ok=True)
    ...
    print(f'DONE downloaded={len(records)} failed={len(failures)}')
