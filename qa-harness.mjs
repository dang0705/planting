...
async function attr(n, k) {
  try {
    return await n.attribute(k)
  } catch {
    return null
  }
}
async function txt(n) {
  try {
    return await n.text()
  } catch {
    return null
...
async function ensureAuto() {
  try {
    ...
    log('port', PORT, 'already listening')
  } catch {
    auto = spawn(CLI, ['auto', '--project', PROJECT, '--auto-port', String(PORT)], {
      stdio: 'ignore'
    })
...
async function flowIndex() {
  ...
  if (diagBtn) {
    try {
      ...
      out.diagnoseErr = String(e?.message || e)
    }
  }
  ...
  if (waterBtn) {
    ...
    try {
      ...
      if (potRow.node) {
        ...
        out.potProfile = {
          sheet: potSheet.found,
          confirmBtn: potConfirm.found,
          closeBtn: potClose.found
        }
        await mp.screenshot({ path: `${SHOT_DIR}/popup-pot-profile-open.png` })
        if (potClose.node) {
          await potClose.node.tap()
          await sleep(1200)
        }
      }
      // open date picker
      ...
      out.waterErr = String(e?.message || e)
    }
  }
  return out
}
...
async function flowPlantDetail() {
  ...
  out.rootStyle = root
  const diagBtn = await existsVisible(page, 'user-plant-detail-diagnose-button')
  ...
  if (editBtn) {
    try {
      ...
      out.actionSheetErr = String(e?.message || e)
    }
  }
  // open diagnose popup
  if (diagBtn.node) {
    try {
      ...
      out.pdDiagnoseErr = String(e?.message || e)
    }
  }
  return out
}
