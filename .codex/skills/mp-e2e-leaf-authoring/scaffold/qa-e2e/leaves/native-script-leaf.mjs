...
/**
 * Project-side native leaf adapter. The leaf is registered through defineLeaf
 * and receives only LeafContext. Existing Node scenario scripts stay isolated
 * so the reusable package never discovers project leaves.
 */
...
async function runNativeScenario({
  ...
  if (!projectRoot || !scriptPath.startsWith(`${projectRoot}${path.sep}`)) {
    const error = new Error(
      `native leaf ${id} has an invalid project script path`
    );
    error.code = "mp_e2e_contract_invalid";
    throw error;
  }
  ...
  if (
    ...
    const error = new Error(
      result.error?.message ||
        legacyReport?.blockerReason ||
        `native leaf ${id} failed with exit ${result.code ?? "unknown"}`
    );
    error.code = result.timedOut
      ? "mp_e2e_script_leaf_timeout"
      : legacyReport?.classification === "BLOCKED_FIXTURE" ||
          legacyReport?.failure_kind === "blocked_fixture"
        ? "mp_e2e_fixture_leaf_blocked"
        : legacyReport?.classification === "FAIL_PRODUCT" ||
            legacyReport?.failure_kind === "failed_product" ||
            legacyReport?.status === "failed"
    ...
    error.details = {
      ...
      code: result.code,
      signal: result.signal,
      legacyReport: summarizeReport(legacyReport),
    };
    throw error;
  }
  return {
    ...
    assertions: [
      {
        name: "native_leaf_exit",
        passed: true,
        evidence: {
          executionPath,
          code: result.code,
          report: summarizeReport(legacyReport),
        },
      },
    ],
  };
...
function spawnScenario(scriptPath, { cwd, env, timeoutMs }) {
  return new Promise((resolve) => {
    ...
    const append = (value, chunk) =>
      value.length >= MAX_LOG_BYTES
        ? value
        : `${value}${String(chunk)}`.slice(0, MAX_LOG_BYTES);
    const finish = (result) => {
      if (settled) return;
      settled = true;
      clearTimeout(timer);
