...
**不要**在读完本页后再默认打开官网。仅当目标包 `vitest` 主版本的 API、`vitest.config.*` 或报错无法用源码/邻近测试解释时，再拉上述 URL（或该主版本的文档）。
...
- Hook/mutation 工厂：**把调用方 options 传入** mock，例如  
  `useSubmitMutation: (opts) => mockSubmit(opts)`。  
  写成 `() => mockSubmit()` 时，产品注册的 `onError`/`onSuccess` 丢失。
