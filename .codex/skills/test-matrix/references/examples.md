...
需要结构或交付格式时读本文件。标识符换成当前仓库模块即可。用语不明再查 [jargons.md](jargons.md)，本文件不挂钩用语。

下列代码块**只演示局部写法**，不代表完整合格 suite。是否合格看 `SKILL.md` §0 + §2.1：Expected 独立且适用维已写、不适用已 N/A。防浅测细则见 [validity.md](validity.md)。真相源见 [truth-sources.md](truth-sources.md)。
...
### 对：列表 mapper 的 U1「数组元素洞」（强制；曾漏测样板）

**漏测形态（禁止再犯）：** Happy 只喂完整 DTO；Edge 只测「字段为 null」（如 `special_price: null`）或「数组缺失 → `[]`」；页面测把 related/list query **整段 mock 掉**或 mock 成干净数组。运行时若 API 返回 `[valid, null, …]`，`.map((item) => item.special_price)` 会抛 `Cannot read properties of null (reading '…')`，矩阵全绿也拦不住。

**Common scene：** U1 空缺 → 集合**元素洞**（见 [jargons.md](jargons.md) `u-dims`）。真相源：DTO 可选元素 + 脏列表容错（契约未保证 items 元素非空时，前端不得假设）。

```ts
import { describe, expect, test } from "vitest";
import { mapRelatedProductList } from "./related.mapper";

describe("mapRelatedProductList", () => {
  test("happy: maps price from special_price when present", () => {
    expect(
      mapRelatedProductList([
        {
          id: 1,
          sku: "A",
          name: "A",
  ...
  describe("edge / reverse", () => {
    // 字段空 ≠ 元素洞；两条都要，不能互相顶替
    test("U2: special_price null falls back to price", () => {
      expect(
        mapRelatedProductList([
          {
            id: 1,
    ...
    test("U1: null/undefined elements are skipped without throw", () => {
      expect(() =>
        mapRelatedProductList([
          null,
          {
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| mapRelatedProductList | L1 | Happy | prefers special_price | DTO 价格字段 | 已写 |
| mapRelatedProductList | L1 | U2 | field special_price null | DTO 可空字段 | 已写 |
| mapRelatedProductList | L1 | U1 | null elements skipped | common scene 元素洞 | 已写 |

**错（本次事故同构）：**

```ts
// 错 1：以为测了 special_price null 就覆盖了脏列表
test("special_price null", () => {
  expect(mapOne({ ..., special_price: null }).price).toBeTruthy();
});

// 错 2：页面把 list query mock 成干净数据，却宣称 related 映射已验收
useRelatedProducts.mockReturnValue({ data: [validDto] }); // 从未喂 [null, dto]

// 错 3：只写 (data ?? []).map(fn) 的 characterization，不断言「不抛 / 跳过洞」
```

**对：** 列表入口（`mapXList` / `compactDefined` 后再 map）必须有 **元素洞** 独立 case；页面若直接 `.map(DTO)`，要么改走 list 入口，要么 L2 喂 `[null, dto]` 断言不崩且展示条数正确。

### 错：实现镜像（自己测自己）

```ts
const expected = mapList(dto);
expect(mapList(dto)).toEqual(expected);
```

Happy 的字段期望须手写，并来自 DTO/接口文档/制品，不是再跑一遍被测函数。
...
```tsx
describe("Reverse 硬度", () => {
  test("submit onSuccess → success page", () => {
    onSubmitSuccess?.({ id: 1, result: "success" });
    expect(navigate).toHaveBeenCalledWith("success", expect.anything());
  }); // 这是 Happy，不得放在 Reverse 块
});
```

对：`describe("Happy: 提交成功")` / `describe("Reverse: 提交失败与取消")` 分开；Reverse 内只留 cancel/fail/onError/缺字段且断言副作用未发生。

### 错：只点 disabled；或把 Happy 塞进 reverse 分组

```tsx
// 错 1：按钮已 disabled，点击几乎恒真；handler 内防御 toast 从未执行
test("submit blocked", async () => {
  render(<SubmitButton disabled onSubmit={onSubmit} />);
  await user.click(screen.getByRole("button", { name: "提交" }));
  expect(onSubmit).not.toHaveBeenCalled();
});
...
describe("edge / reverse", () => {
  test("card opens detail", async () => {
    await user.click(screen.getByText("记录编号"));
    expect(navigate).toHaveBeenCalled(); // 这是 Happy
  });
});
```

对：另开「强制调用」入口触达 handler 防御 toast，并断言 `navigate` 未调用；Happy 导航放在 `describe("happy")`（或无 reverse 标签的块）。

### 错：page 壳 mock 掉编排 hook 却声称 CTA 已覆盖

```tsx
vi.mock("./useProgressPage", () => ({
  useProgressPage: () => ({ progress: vm, handlePrimaryCta: vi.fn() }),
}));
test("progress page", () => {
  render(<ProgressPage />);
  expect(screen.getByText("处理中")).toBeInTheDocument();
  // 未证明空字段 toast / cancel onError
});
```

对：壳测只报门闸；CTA Reverse 写在 `useProgressPage` 单测（或解开 mock）。
...
停工：闸门 1 冻结控制流；要 peel 必须先有「点 tab 后选中 id 保持」的红/绿宿主测（闸门 2–3）。测不了 Tabs → 加厚 mock 的 `onChange`，或标缺口，**不许改 page 迁就**。
...
describeLive("list contract (optional live)", () => {
  test("I1: list DTO maps through mapList", async () => {
    const raw = await fetch(`${process.env.API_BASE_URL}/items`).then((r) =>
      r.json()
    );
    const page = mapList(raw);
    expect(page.items.length).toBe(raw.items.length);
  });
});
```

证明范围须写明：经过 `mapList`；**未经过**产品 HTTP 封装时不得声称拦截器/鉴权已测。
...
```ts
const raw = { items: [{ id: 1, status: "ready" }] };
expect(mapList(raw).items[0].status).toBe("ready");
```
