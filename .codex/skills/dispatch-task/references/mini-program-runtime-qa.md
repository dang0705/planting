...
8. live `qa-run` 必须先持久化已通过的 preflight：target projectPath、完整 LAN flow、正式 QA 的 `9421` WebSocket 与 `9422` DevTools control port、page data、截图与小程序运行时 `wx.request`。`9420` 只属于用户日常交互调试，不得出现在正式 `qa-run` evidence。execution record 必须保存 frozen hash、preflight 和终态；同 hash 最多两次 live attempt，产品失败不得原地重试。
9. DevTools 当前项目不等于 target projectPath 时不得重启。正式 QA 截图 RPC 失效时，只有从 QA-owned `9422` control port、`9421` WebSocket、owner PID、profile 和 projectPath 共同证明唯一目标，才可一次执行 target-only `close -> open -> auto`；不得从日常 `9420` listener、日常 PID 或日常 profile 推导正式 QA ownership。必须记录每次 CLI 调用、前后 QA main PID / `9421` listener PID / `9422` control port / projectPath，以及恢复后的截图和 `wx.request`。`--port` 是 IDE 控制端口，不能把 `9421` 或不存在的 `--auto-port` 当作控制端口；任一证明缺失以 `devtools_automator_blocker` 失败。
