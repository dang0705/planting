...
4. 运行 `zcode-clipboard-bridge.mjs`。只有 bridge evidence 为 `prepared`，且 SHA-256、bytes、lines 在本地读回一致，才继续；这些字段不要求 ZCode UI 再次暴露。
5. 在已聚焦的 entry area 执行系统 Cmd+V。
6. 读取最新 app state，验证 `direct_text` 或 `pasted_text_attachment` 已可见；附件分支只需确认可见行数等于 canonical prompt 行数，不要求 UI 提供 hash/bytes。
7. 只有本地 bridge 已验证、输入框焦点已验证且粘贴内容可见后，才点击发送。Cmd+V 是唯一正式粘贴路径。
8. 读取发送后的最新 app state，证明输入已提交、会话状态发生变化、消息或同一附件进入当前会话。
9. 记录 `sent` 后断开 Computer Use；后续 recovery 遵循 external implementer 公共合同。

每次 app state 后取得的 element index 只在该 state 内有效。禁止跨 state 复用或把 index 写入 receipt。Cmd+V 无可见交付时直接 blocked，不得进入 Edit 菜单、headless、新会话、手输或逐字输入。
...
必须保留本地 bridge 的 canonical SHA-256/bytes/lines 证据，并观察附件名称和行数；UI 不需要重复提供 hash/bytes。发送前附件可见，发送后同一附件仍在当前会话消息中。sentinel 不要求可见，也不得伪造其可见性。
...
- `paste_delivery.pre_send_verified=true`，Cmd+V 可见成功即可；
- `prompt_identity.verified_before_after=true` 表示本地 canonical identity 与 bridge readback 一致，且 UI 可见行数未变化，不表示 UI 重复暴露 hash/bytes；
