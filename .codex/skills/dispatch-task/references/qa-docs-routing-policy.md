...
2. BRV Fact Routing Packet、task facts、ClickUp checklist、代码注释或活文档明确要求同步 docs / new-rules / source index / BRV source verification。
...
QA 只做 e2e、端上测试、UI/Figma 验收、运行时链路取证与失败归因；QA 不运行单测、不重复 implementer 单测、不审代码 diff、不替代 main agent code review。
