...
external_contract.required_computer_use_actions = dynamic focus -> bridge -> Cmd+V -> verify -> send -> post-send verify
...
这些字段由 main 生成到 `external_contract`。bridge 先从 canonical prompt regular file 读取 UTF-8 正文，按 NSPasteboard → pbcopy 写入并逐次读回 SHA-256/bytes/lines；未验证就继续下一方法。main 随后用最新 ZCode app state 动态聚焦唯一输入区，只执行一次 Cmd+V，并用可见文本或附件行数确认交付；发送前后都必须验证当前会话实际收到完整 prompt。任何失败只返回 blocked，不得进入 Edit 菜单、headless、新会话、手输或逐字输入。
