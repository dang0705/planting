...
import { handleHookEvent, inspectHookCapability } from './hook-events.mjs'
...
import { repoRoot, stateDir, writeJsonAtomic } from './state.mjs'

export function hookSelfTest({ emit }) {
  ...
  const agentId = `hook-self-test-provider-${Date.now()}`
  const bound = bindAgentToEpisode({ dispatchRunId, agentId, agentType: 'external_implementer' })
  const payload = {
    dispatch_run_id: dispatchRunId,
    agent_id: agentId,
...
export function episodeCommand({ args, argValue, hasFlag, emit }) {
  const action = args[0]
  ...
  if (action === 'rework') {
    ...
    const values = {
      ...
      userDecisionReason: argValue('user-decision-reason'),
      fixTarget: argValue('fix-target'),
      relationship: argValue('relationship')
    }
    for (const [key, value] of Object.entries(values)) {
      if (value) supersedesTarget[key] = value
    }
  ...
  if (action === 'migrate-legacy-lifecycle') {
    ...
    return emit(result, ['migrated', 'already_migrated'].includes(result.status) ? 0 : 1)
  }
