...
async function runCommand(name, argv2) {
...
import { chmod as chmod9, mkdir as mkdir21, open as open6, readFile as readFile29, rm as rm20 } from "node:fs/promises";
...
async function removePid(daemonDir2) {
  await rm20(join32(daemonDir2, FILE2), { force: !0 });
}
...
shouldEnsureDaemonForBrvCommand(argv[0]) && await ensureDaemon();
await main(argv);
