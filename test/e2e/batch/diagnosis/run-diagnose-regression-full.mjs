...
async function main() {
  ...
  const visualManifestPath = path.resolve(
    process.cwd(),
    args['visual-manifest'] || 'test/e2e/batch/diagnosis/manifests/diagnose-visual-regression.manifest.json'
  )
  const outcomeManifestPath = path.resolve(
    process.cwd(),
    args['outcome-manifest'] || 'test/e2e/batch/diagnosis/manifests/diagnose-outcome-regression.manifest.json'
  )
  const fastConvergenceManifestPath = path.resolve(
    process.cwd(),
    args['fast-convergence-manifest'] ||
      'test/e2e/batch/diagnosis/manifests/high-specificity-fast-convergence.manifest.json'
  )
  const visualReportPath = path.resolve(
    process.cwd(),
    args['visual-report-file'] || 'test/e2e/batch/diagnosis/manifests/diagnose-visual-regression.report.json'
  )
  const outcomeReportPath = path.resolve(
    process.cwd(),
    args['outcome-report-file'] || 'test/e2e/batch/diagnosis/manifests/diagnose-outcome-regression.report.json'
  )
  const fastConvergenceReportPath = path.resolve(
    process.cwd(),
    args['fast-convergence-report-file'] ||
      'test/e2e/batch/diagnosis/manifests/high-specificity-fast-convergence.report.json'
  )
