...
const scriptUrl = pathToFileURL(path.join(repoRoot, 'scripts/dev/run-with-cloudbase-env.mjs')).href
assert.equal(resolveProjectRoot(scriptUrl), repoRoot, '从 scripts/dev 应解析到仓库根')

// 2. 从更深的 test/e2e/batch workflow 文件解析也必须命中仓库根。
const testUrl = pathToFileURL(
  path.join(repoRoot, 'test/e2e/batch/workflow/cloudbase/run-with-cloudbase-env-project-root.mjs')
).href
assert.equal(resolveProjectRoot(testUrl), repoRoot, '从 test/e2e/batch workflow 应解析到仓库根')
