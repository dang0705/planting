...
const visibleZcodeContract = JSON.parse(
  fs.readFileSync(
    path.join(repoRoot, '.codex/skills/dispatch-task/examples/zcode-external-ui-handoff.json'),
    'utf8'
  )
).external_contract
const visibleReceipt = (runId, status = 'sent') => {
  const promptPath = path.join(repoRoot, '.tmp', 'dispatch-task', `${runId}-prompt.md`)
  const prompt = `canonical prompt fixture for ${runId}\n`
  ...
  const identity = {
    ...
    lines: countUtf8Lines(prompt),
    verified_before_after: sent
  }
  return {
    dispatch_run_id: runId,
    status,
    blocker: sent ? null : 'clipboard_write_failed',
    transport: {
      kind: 'zcode_visible_clipboard',
      target_session: 'current_open_chat',
      status: sent ? 'sent' : 'not_started',
      fallback: 'none',
      headless_used: false
    },
    ...
    clipboard: {
      write_attempts: [
        {
          method: 'nspasteboard',
          status: sent ? 'verified' : 'write_failed',
          readback_verified: sent,
          attempted_at: '2026-07-30T01:00:00.000Z'
        }
      ],
      selected_method: sent ? 'nspasteboard' : null,
      readback_verified: sent,
      verified_at: sent ? '2026-07-30T01:00:01.000Z' : null
    },
    input_focus: {
      latest_app_state_checked: sent,
      unique_entry_area: sent,
      clicked: sent,
      focused_element_verified: sent,
      element_index_persisted: false,
      verified_at: sent ? '2026-07-30T01:00:02.000Z' : null
    },
    paste_delivery: {
      attempts: [{ method: 'cmd_v', delivery_verified: sent }],
      selected_method: sent ? 'cmd_v' : null,
      rendering: sent ? 'direct_text' : null,
      pre_send_verified: sent,
      direct_text_identity: sent
        ? {
        ...
        : null,
      attachment: null,
      verified_at: sent ? '2026-07-30T01:00:03.000Z' : null
    },
    send_delivery: {
      send_clicked: sent,
      input_submitted: sent,
      conversation_state_changed: sent,
      conversation_delivery_verified: sent,
      verified_at: sent ? '2026-07-30T01:00:04.000Z' : null
    },
    redaction: {
    ...
    sent_at: sent ? '2026-07-30T01:00:05.000Z' : null
  }
}
...
const recordedProductAttempt = JSON.parse(fs.readFileSync(productRecord, 'utf8'))
assert.equal(recordedProductAttempt.live_attempt, 1)
assert.equal(recordedProductAttempt.live_attempt_consumed, true)
// qa-run must now complete the independent runtime preflight before checking a
// product retry. The executable retry rejection belongs in qa-run.mjs, where it
// is tested with injected preflight/leaf runners and never touches DevTools.
...
writeJson(conflictHandoff, {
  ...
  task: {
    objective: 'path conflict regression',
  ...
  project_constraints: {
    ...
    test_commands: ['node test/e2e/batch/workflow/dispatch-gate-contract/qa-and-validation.mjs']
  },
  ...
  feature_test_plan: {
    required: true,
    targets: ['test/e2e/batch/workflow/dispatch-gate-contract/qa-and-validation.mjs'],
    commands: ['node test/e2e/batch/workflow/dispatch-gate-contract/qa-and-validation.mjs']
  },
  ...
  validation: {
    ...
    worktree_baseline_path: `/tmp/${conflictRunId}-baseline.json`
  },
  selection_to_consumer: {
    required: false,
    not_applicable_reason: 'validator contract fix has no user-selectable values'
  }
})
const conflictValidation = spawnSync(process.execPath, [handoffValidator, conflictHandoff], {
  cwd: repoRoot,
  encoding: 'utf8'
})
...
const okConflictValidation = spawnSync(process.execPath, [handoffValidator, okConflictHandoff], {
  cwd: repoRoot,
  encoding: 'utf8'
})
...
const okConflictErrors = Array.isArray(okConflictParsed.errors)
  ? okConflictParsed.errors.join('\n')
  : ''
assert.doesNotMatch(
  okConflictErrors,
  /allowed_paths and forbidden_paths conflict/,
...
const blockedRecoveryHandoff = path.join(
  '.tmp',
  'dispatch-task',
  `${blockedRecoveryRunId}-handoff.json`
)
writeJson(blockedRecoveryHandoff, {
  ...
  task: {
    objective: 'blocked recovery regression',
    code_changes_required: true,
    ui_task: false,
    risk: 'standard',
    qa_required: false
  },
  external_contract: visibleZcodeContract,
...
const blockedRecoveryResult = path.join(
  '.tmp',
  'dispatch-task',
  `${blockedRecoveryRunId}-result.json`
)
writeJson(blockedRecoveryResult, {
  source: 'codex_recovery_after_external',
  status: 'blocked',
  codex_self_implementation: false,
  external_completion_claim_treated_as_non_authoritative: true,
  git_diff_recovered_by_codex: true,
  allowed_forbidden_paths_checked: true,
  project_constraints_checked_by_codex: true,
  external_handoff_manual: {
    read_by_codex: true,
    path: `/tmp/${blockedRecoveryRunId}-manual.json`,
    status: 'missing',
    updated_at: ''
  },
  external_send_receipt: visibleReceipt(blockedRecoveryRunId, 'blocked'),
  ...
  selection_to_consumer: {
    not_applicable: true,
    reason: 'validator contract fix has no user-selectable values'
  }
})
const blockedRecoveryValidation = spawnSync(
  process.execPath,
  [resultValidator, 'external', blockedRecoveryHandoff, blockedRecoveryResult],
  { cwd: repoRoot, encoding: 'utf8' }
)
assert.equal(
  blockedRecoveryValidation.status,
  0,
  `blocked external recovery with forbidden changed_file must pass result-contract: ${blockedRecoveryValidation.stderr || blockedRecoveryValidation.stdout}`
)
...
const completedRecoveryResult = path.join(
  '.tmp',
  'dispatch-task',
  `${blockedRecoveryRunId}-completed-result.json`
)
writeJson(completedRecoveryResult, {
  ...
  allowed_forbidden_paths_checked: true,
  project_constraints_checked_by_codex: true,
  external_recovery_evidence: {
    handoff_manual_read: true,
  ...
  external_send_receipt: visibleReceipt(blockedRecoveryRunId),
  changed_files: ['src/forbidden/file.js'],
...
const completedRecoveryValidation = spawnSync(
  process.execPath,
  [resultValidator, 'external', blockedRecoveryHandoff, completedRecoveryResult],
  { cwd: repoRoot, encoding: 'utf8' }
)
assert.notEqual(
  completedRecoveryValidation.status,
  0,
  'completed external recovery with forbidden changed_file must still be rejected'
)
const completedErrors = JSON.parse(
  completedRecoveryValidation.stdout || completedRecoveryValidation.stderr
).errors.join('\n')
assert.match(
  completedErrors,
  /changed file matches forbidden_paths|changed file outside allowed_paths/
)
...
writeJson(globVsGlobHandoff, {
  ...
  implementation_mode: 'main_direct',
  task: {
    objective: 'glob vs glob regression',
    code_changes_required: true,
    ui_task: false,
    risk: 'high',
    qa_required: true
  ...
  validation: {
    miniprogram_automator_required: true,
    runtime_acceptance_mode: 'automator_required',
    worktree_baseline_path: `/tmp/${globVsGlobRunId}-baseline.json`
  },
  selection_to_consumer: {
    required: false,
    not_applicable_reason: 'validator contract fix has no user-selectable values'
...
const globVsGlobValidation = spawnSync(process.execPath, [handoffValidator, globVsGlobHandoff], {
  cwd: repoRoot,
  encoding: 'utf8'
})
assert.notEqual(
  globVsGlobValidation.status,
  0,
  'glob-vs-glob overlap (a/*.js vs a/foo*.js) must be rejected before dispatch'
)
...
const missedSampleValidation = spawnSync(
  process.execPath,
  [handoffValidator, missedSampleHandoff],
  { cwd: repoRoot, encoding: 'utf8' }
)
assert.notEqual(
  missedSampleValidation.status,
  0,
  'exact glob intersection must catch p/*a vs p/z* overlap'
)
assert.match(
...
const doubleStarValidation = spawnSync(process.execPath, [handoffValidator, doubleStarHandoff], {
  cwd: repoRoot,
  encoding: 'utf8'
})
...
assert.match(
  JSON.parse(doubleStarOutput).errors.join('\n'),
  /allowed_paths and forbidden_paths conflict/
)
...
const deliveredManualHandoff = path.join(
  '.tmp',
  'dispatch-task',
  `${deliveredManualRunId}-handoff.json`
)
writeJson(deliveredManualHandoff, {
  ...
  implementation_mode: 'external_implementer',
  task: {
    objective: 'delivered manual regression',
    code_changes_required: true,
    ui_task: false,
    risk: 'standard',
    qa_required: false
...
const deliveredManualResult = path.join(
  '.tmp',
  'dispatch-task',
  `${deliveredManualRunId}-result.json`
)
...
writeJson(deliveredManualResult, {
  ...
  external_handoff_manual: {
    read_by_codex: true,
    path: `/tmp/${deliveredManualRunId}-manual.json`,
    provider_status: 'delivered',
    updated_at: '2026-07-27T22:00:00Z'
  },
  external_send_receipt: visibleReceipt(deliveredManualRunId),
...
const deliveredManualValidation = spawnSync(
  process.execPath,
  [resultValidator, 'external', deliveredManualHandoff, deliveredManualResult],
  { cwd: repoRoot, encoding: 'utf8' }
)
assert.equal(
  deliveredManualValidation.status,
  0,
  `completed external recovery with provider_status=delivered manual must pass result-contract: ${deliveredManualValidation.stderr || deliveredManualValidation.stdout}`
)
...
const legacyManualResult = path.join(
  '.tmp',
  'dispatch-task',
  `${deliveredManualRunId}-legacy-result.json`
)
...
const legacyManualValidation = spawnSync(
  process.execPath,
  [resultValidator, 'external', deliveredManualHandoff, legacyManualResult],
  { cwd: repoRoot, encoding: 'utf8' }
)
assert.equal(
  legacyManualValidation.status,
  0,
  `completed external recovery with legacy status=completed manual must stay compatible: ${legacyManualValidation.stderr || legacyManualValidation.stdout}`
)
// Negative: provider_status=blocked manual must not satisfy completed recovery.
const blockedManualResult = path.join(
  '.tmp',
  'dispatch-task',
  `${deliveredManualRunId}-blocked-manual-result.json`
)
...
const blockedManualValidation = spawnSync(
  process.execPath,
  [resultValidator, 'external', deliveredManualHandoff, blockedManualResult],
  { cwd: repoRoot, encoding: 'utf8' }
)
assert.notEqual(
  blockedManualValidation.status,
  0,
  'completed recovery with provider_status=blocked manual must be rejected'
)
const blockedManualErrors = JSON.parse(
  blockedManualValidation.stdout || blockedManualValidation.stderr
).errors.join('\n')
...
const unknownProjectPreflight = await runQaPreflight({
  ...
  runtimeInspector: () => ({
    ...
    automator_listener_pids: ['unverified-listener'],
    automator_port: 9420,
    control_port: 9420,
    project_evidence: []
  }),
...
assert.equal(persistedLeafReport.business_assertions_reached, true)
...
const governanceValidation = spawnSync(
  process.execPath,
  [resultValidator, 'main', governanceHandoff, governanceResult],
  {
    cwd: repoRoot,
    encoding: 'utf8'
  }
...
cleanupDispatchState(blockedRecoveryRunId)
cleanupDispatchState(deliveredManualRunId)
