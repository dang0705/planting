 ...
 *
 * 用法：
 *   node test/e2e/automator/care/watering/transpiration-v3/<leaf>.mjs [--watering-transpiration-mode=shadow|active]
 *
 * 环境变量：
...
export async function runTranspirationV3({ forcedScenario = null } = {}) {
  const argv = process.argv.slice(2)
  const env = resolveEnv(argv)
  const scenario = forcedScenario || parseScenario(argv)
