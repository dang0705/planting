...
import { navigateNativeTab } from '../../../diagnosis/_shared/native-tab-navigation.mjs'
import automator from 'miniprogram-automator'
import fs from 'node:fs'

const WS_ENDPOINT = formalAutomatorEndpoint()
...
;(async () => {
  ...
  let mp = (await connectFormalLeaf({ automator, wsEndpoint: WS_ENDPOINT })).mp
  console.log('[1] ✓ automator 已连接')

  try {
    await navigateNativeTab({ mp, logicalPath: '/pages/index/index' })
    await sleep(3000)
    ...
    mp = (
      await handoffFormalLeafScreenshot({
        ...
        outputPath: `${SCREENSHOT_DIR}/01-date-picker.png`
      })
    ).mp
    ...
    mp = (
      await handoffFormalLeafScreenshot({
        ...
        automator,
        wsEndpoint: WS_ENDPOINT,
        outputPath: `${SCREENSHOT_DIR}/02-after-date-select.png`
      })
    ).mp
    ...
    mp = (
      await handoffFormalLeafScreenshot({
        ...
        automator,
        wsEndpoint: WS_ENDPOINT,
        outputPath: `${SCREENSHOT_DIR}/03-planner-result.png`
      })
    ).mp
    ...
    mp = (
      await handoffFormalLeafScreenshot({
        ...
        automator,
        wsEndpoint: WS_ENDPOINT,
        outputPath: `${SCREENSHOT_DIR}/04-final.png`
      })
    ).mp
    await mp.disconnect()
