...
async function run() {
  ...
  const hasCatalog = firstUrls.some(
    u => u.includes('plant-catalog') || u.includes('catalog/plants') || u.includes('catalog')
  )
  ...
  log(
    'POINT1_RESULT: ' +
      (hasCatalog && !hasHotCities && !hasHotCitiesResolve && !hasAuthUser && !hasUserPlants
        ? 'PASS'
        : 'FAIL')
  )
  ...
  const hasAfterBorder0 =
    nextBtnInfo && nextBtnInfo.className && nextBtnInfo.className.includes('after_cborder-0')
  log(
    'POINT4_CHECK: after_cborder-0_present=' +
      hasAfterBorder0 +
      ' disabled=' +
      (nextBtnInfo ? nextBtnInfo.disabled : 'N/A')
  )
  ...
  if (plantSelected) {
    ...
    log(
      'POINT2_CHECK: hasHotCities=' +
        hasHotCitiesAfter +
        ' hasHotCitiesResolve=' +
        hasHotCitiesResolveAfter
    )
  ...
  try {
    ...
    const hasUserPlantsEdit = editUrls.some(
